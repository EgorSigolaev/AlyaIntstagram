...
class AdvertImageViewHolder(itemView: View) : RecyclerView.ViewHolder(itemView) {
    ...
    fun bind(post: Post){
        ...
        textViewPostLikes.text = "${context.getString(R.string.like)}: ${(post.postType as Posts.AdvertImage).likes}"
        ...
        recyclerViewPost.adapter = PostImagesAdapter(urls = (post.postType as Posts.AdvertImage).imagesUrls, postType = post.postType!!)
